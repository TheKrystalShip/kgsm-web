...
Overview, Reach and Capabilities, over URL segments the way every other tabbed page in the panel
works: a tab moves the URL, Back walks between them, and a link opens into one. The three tab words
are reserved in the first segment after `/cluster`, so a member with one of those ids is shadowed —
the same trade `/library/new` makes.

**Overview** opens with six figures the page had no way to state: how many members are answering,
how many capabilities are served, the slowest link, the busiest node, what is running, and whether
every member is on the same build. Each names what it measured or says it could not; none of them
reports a zero for a fact nobody could establish.

**Reach** is one axis measured from this browser, cut into three bands of fixed width. The scale is
banded rather than continuous because a continuous one is ruled by its furthest member: a single
peer over a slow link squeezes every LAN member into the first few pixels. Below the layout
breakpoint the axis becomes sections and the members become rows, since a position inside a band is
not readable at phone width.

**Reach also carries a map** — Natural Earth coastlines projected the way the EU projects its own
statistics, generated by `scripts/gen-eu-map.mjs` into a module and loaded with the tab rather than
with every first paint. It draws a pin only for a member that reports latitude and longitude and
names every member that does not. Members in one place are one pin whose callout lists them.

**Capabilities** is the only surface on the page that spans both kinds of member, because a
capability belongs to the cluster rather than to a machine. Four cell states, and only one is a
claim about ability: a member offers a capability when its own capability block says so, and says
nothing at all where nothing has been reported — a grid of confident refusals nothing measured
would be worse than a grid of dashes.

### Changed — a member row states its identity, and hover belongs to the page

Both member cards carry the id under the display name, with whatever else the member reported about
itself, and a node states the capability it serves for the cluster. The id is dropped when nobody
has renamed the member, since printing the same word twice says nothing about it.

`hoveredNode` is the page's and every surface takes it as a prop, so pointing at a member in the
reach rail lights it in the map's callout beside it and in the Nodes card on the tab before.

A round trip under a millisecond now reads `<1ms` rather than `0ms`, everywhere one is shown.
